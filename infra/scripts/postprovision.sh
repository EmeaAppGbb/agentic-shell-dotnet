...
if command -v jq &> /dev/null; then
    ...
    jq --arg endpoint "$OPENAI_ENDPOINT" --arg deployment "$OPENAI_DEPLOYMENT" --arg imageModel "$IMAGE_MODEL_DEPLOYMENT" \
        --arg cosmosName "$COSMOS_NAME" --arg cosmosResourceGroup "$COSMOS_RESOURCE_GROUP" \
        '.Parameters.openAiEndpoint = $endpoint | .Parameters.openAiDeployment = $deployment | .Parameters.imageModelDeployment = $imageModel | .Parameters.cosmosName = $cosmosName | .Parameters.cosmosResourceGroup = $cosmosResourceGroup' \
    ...
    sed -i.bak "s|\"imageModelDeployment\".*:.*\".*\"|\"imageModelDeployment\": \"$IMAGE_MODEL_DEPLOYMENT\"|g" "$SETTINGS_FILE"    sed -i.bak "s|\"cosmosName\".*:.*\"\.\*\"|\"cosmosName\": \"$COSMOS_NAME\"|g" "$SETTINGS_FILE"
    sed -i.bak "s|\"cosmosResourceGroup\".*:.*\"\.\*\"|\"cosmosResourceGroup\": \"$COSMOS_RESOURCE_GROUP\"|g" "$SETTINGS_FILE"    rm -f "$SETTINGS_FILE.bak"
fi
...
echo -e "\033[0;36m  - Cosmos DB Name: $COSMOS_NAME\033[0m"
echo -e "\033[0;36m  - Cosmos DB Resource Group: $COSMOS_RESOURCE_GROUP\033[0m"
